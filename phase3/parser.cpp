...
{
    if (lookahead == DONE)
	report("syntax error at end of file", lexbuf);
    else
...
static string identifier(int token) {
	string buf = lexbuf;
	match(token);
	return buf;
}
...
static void declarator(int typespec) {
	...
	if (lookahead == '[') {
		match('[');
		unsigned long len = stoul(lexbuf, nullptr);
		match(NUM);
		match(']');
		Type t = Type(typespec, indirection, len);
		cout << "(" << t << ", ARRAY)" << endl;
	} 
	else {
...
static Type parameter()
{
	...
	const string name = identifier(ID);
	const Type t = Type(typespec, indirection);
...
static Parameters *parameters()
{
	...
	string name = identifier(ID);
	const Type type = Type(typespec, indirection);
...
static void globalOrFunction()
{
    ...
    if (lookahead == '[') {
        match('[');
        unsigned long len = stoul(lexbuf);
		match(NUM);
		Type t = Type(typespec, indirection, len);
		declareVariable(name, t);
        match(']');
        ...
        match('(');
		Type t = Type(typespec, indirection, nullptr);
